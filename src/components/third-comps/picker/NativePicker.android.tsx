...
class Picker extends React.Component<IPickerProp & PickerProps, any> {
  ...
  scrollerRef: ScrollView | null = null
  contentRef: View | null = null
  indicatorRef: View | null = null
    ...
    const items = React.Children.map(children, (item: any, index) => {
      ...
      return (
        <View
          ...
          style={styles.pickerItemView}
          key={item.key}
        >
          <Text style={[{ includeFontPadding: false }, totalStyle, itemStyle, styles.pickerItemText]} numberOfLines={1}>
            {item.props.label}
          </Text>
        </View>
      )
