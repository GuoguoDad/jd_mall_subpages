...
export default class Picker extends React.Component<PickerProps, any> {
  ...
  getPickerCol = () => {
    ...
    return (data as PickerData[][]).map((col, index) => {
      return (
        <RMCPicker key={index} style={{ flex: 1 }} itemStyle={itemStyle} indicatorStyle={indicatorStyle}>
          {col.map(item => {
            return (
  ...
  render() {
    ...
    const _locale = getComponentLocale(this.props, (this as any).context, 'Picker', () => require('./locale/zh_CN'))

    const { cascader, popupMoreProps }: { cascader: JSX.Element; popupMoreProps: {} } = this.getCascade(
      cascade,
      data,
      cols,
      itemStyle,
  ...
  getCascade = (
    ...
    if (cascade) {
      ...
      cascader = (
        <MultiPicker style={{ flexDirection: 'row', alignItems: 'center' }} onScrollChange={this.setScrollValue}>
          {this.getPickerCol()}
        </MultiPicker>
      )
      popupMoreProps = {
